...
- defensive guards around persisted/domain state usually mean the boundary is misplaced
- clean shape: parse untrusted input once at the edge, keep trusted internal values typed, and do not re-normalize internal state downstream
- repeated `?:` and `??` around domain values are a signal to inspect boundaries; if code keeps defaulting or re-normalizing persisted/internal state, validate once at the edge instead
- do not treat all ternaries/nullish coalescing as slop; target defensive fallback chains and repeated coercion, not straightforward business logic
